...
class HyperspaceFX:
    """S' 캔버스에 그리는 전환 스트릭/플래시. start()→step(dt)→render()"""
    ...
    def start(self):
        ...
        self._streaks = []
        for _ in range(180):
            ang = random.uniform(-0.55*math.pi, 0.55*math.pi)
            spd = random.uniform(900, 2200)
            l0  = random.uniform(6, 24)
    ...
    def render(self):
        ...
        for ang, spd, l0 in self._streaks:
            ...
            c.create_line(cx, cy, x2, y2, fill="#bcd7ff", width=2)
        # central flash
        a = int(140 + 100*ease); col = f"#{a:02x}{a:02x}{a:02x}"
        r = int(10 + 120*ease)
