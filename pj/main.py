    ...
    def render_S(self):
        ...
        for cluster in self.clusters:
            for d in cluster:
                ...
                x_vis = (d.x - center_x) / dust_scale
                if d in sel:
                    self.view_S.draw_debris_poly(x_vis, y_px, d.shape_pts, d.theta,
                                                fill="#a7f3d0", outline="#10b981", width=2)
                else:
                    self.view_S.draw_debris_poly(x_vis / c_vis_scale, y_px, d.shape_pts, d.theta,
        ...
        self.view_S.note(f"t = {tS:.2f} s", int(earth_px) - 20, y - 34, fill="#c7d2fe")
        ...
        self.view_S.note(f"τ_ship = {tau_ship:.2f} s", int(ship_px) - 40, y - 34, fill="#fca5a5")
    ...
    def render_Sp(self):
        ...
        for cluster in self.clusters:
            for d in cluster:
                ...
                x_vis = (d.x - center_x) / dust_scale
                if d in sel:
                    self.view_Sp.draw_debris_poly(x_vis, y_px, d.shape_pts, d.theta,
                                                fill="#a7f3d0", outline="#10b981", width=2)
                else:
                    self.view_Sp.draw_debris_poly(x_vis / c_vis_scale, y_px, d.shape_pts, d.theta,
                                                fill="#7c8697", outline="#b6c2d4", width=1)


        # --- 시계 표시 ---
        # 우주선 고유시간 (Twins는 traveler.tau, LC는 train_observer.tau)
        if isinstance(self.ctrl, LCController):
            tau_ship = getattr(self.lc_scn.train_observer.state, "tau", 0.0)
        else:
            tau_ship = getattr(self.tw_scn.traveler.state, "tau", 0.0)

        # 지구 시계 (ICIF simultaneous event의 S-시간 = 지구 고유시간)
        # t_earth_icif = tS - beta * x_ship / C
        tS = self.ctrl.t
        x_ship = self.ctrl.ship_center_x
        t_earth_icif = tS - beta * (x_ship / sim.C)
        if t_earth_icif < 0:  # 수치 오차 보호
            t_earth_icif = 0.0
        tau_earth_icif = t_earth_icif  # 지구는 S에서 정지 ⇒ 고유시간 = 좌표시간

        mP = self.view_Sp._mapper()
        ship_px  = mP.to_px(0.0)                 # S′에서 우주선 중심은 0
        earth_px = mP.to_px(x_earth_p)           # 위에서 구한 x′ 위치(픽셀)

        # 라벨(‘노트’) 표시
        self.view_Sp.note(f"τ_ship = {tau_ship:.2f} s", int(ship_px) - 40,  y - 34, fill="#fca5a5")
        self.view_Sp.note(f"τ_earth = {tau_earth_icif:.2f} s", int(earth_px) - 44, y - 34, fill="#c7d2fe")
        ...
        # 만남(지구와 같은 위치)일 때 시간차 Δ — 두 패널 동일 값이어야 함
        if abs(x_earth_p) < 1e-3 and isinstance(self.ctrl, TwinsController):
