...
class TwinsController(ScenarioController):
    def __init__(self, scn: sim.TwinsScenario, ship_L0: float = 120.0):
        ...
        self.dt = scn.sim.dt
        self._L0 = ship_L0
        self.x_B612 = getattr(scn, "x_turn_m", None)
        if self.x_B612 is None:
            p = scn.params
            self.x_B612 = p.v_out * p.out_duration_s  # L 기본값
    ...
    def earth_x_in_ship_frame(self) -> float:
        ...
        return - x_s / g
    def b612_x_in_ship_frame(self) -> float:
        """현재 우주선 ICIF(순간 관성계)에서 본 B612의 x′ (월드미터)."""
        beta = float(self.beta)
        tS   = float(self.t)
        xS   = float(self.ship_center_x)
        if abs(beta) < 1e-12:
            # 저속/정지 근사
            return self.x_B612 - xS

        boost    = sim.LorentzBoost1D(beta=beta)
        ship_now = sim.Event(ct=sim.C * tS, x=xS)
        ct_p_now = boost.to_other(ship_now).ct      # 우주선 현재 사건의 t′
        g        = sim.gamma(abs(beta))
        # 지구/B612는 S에 정지 ⇒ x′ = x/γ − β·t′
        return (self.x_B612 / g) - (beta * ct_p_now)
